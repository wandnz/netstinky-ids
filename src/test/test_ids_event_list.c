...
void
test_new_ids_event(CuTest *tc)
{
	struct in_addr ip_addr;
	char *iface = "eth0";
	char *ioc = "baddomain.com";

	assert(inet_aton("192.168.1.1", &ip_addr));
	struct ids_event *event = new_ids_event(iface, ntohl(ip_addr.s_addr), strdup(ioc));
	if (event)
	{
...
void
test_new_ids_event_list(CuTest *tc)
{
...
void
test_ids_event_list_add_event(CuTest *tc)
{
	struct ids_event *event = new_ids_event("ether0", 5000, strdup("domain.com")), *old_event;

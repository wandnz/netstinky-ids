...
static void hattrie_split(hattrie_t* T, node_ptr parent, node_ptr node)
{
    ...
    while (j + 1 < node.b->c1) {
        ...
        if (d <= left_m - right_m && left_m + cs[j + 1] < all_m) {
            j += 1;
            left_m  += cs[j];
            right_m -= cs[j];
        }
...
/**
 * Prints the node type of the top item on the stack.
 * @param i
 */
void print_node_type(hattrie_iter_t *i)
{
    assert(i != NULL);
    ...
    // print current key
    if (!i->has_nil_key && (i->i != NULL))
    {
        size_t sublen;
        const char *subkey = ahtable_iter_key(i->i, &sublen);
        size_t key_len;
        const char *key = hattrie_iter_key(i, &key_len);
        printf("%.*s\t%s ", (int)(level - 1), key, subkey);
    }
        ...
        if (*i->stack->node.flag & NODE_TYPE_TRIE)
        {
            printf("NODE_TYPE_TRIE: %u %c", i->stack->c, i->stack->c);
        }
        if (*i->stack->node.flag & NODE_TYPE_PURE_BUCKET)
        {
            printf("NODE_TYPE_BUCKET: %u %c", i->stack->c, i->stack->c);
        }
        if (*i->stack->node.flag & NODE_TYPE_HYBRID_BUCKET)
        {
            printf("NODE_TYPE_HYBRID_BUCKET: %u %c", i->stack->c, i->stack->c);
        }
        if (*i->stack->node.flag & NODE_HAS_VAL)
        {
            printf("NODE_HAS_VAL: %u %c", i->stack->c, i->stack->c);
        }
    }
    printf("\n");
}
...
/**
 * Prints the current location of the iterator and the items in the stack so
 * that a full print out of the HAT trie is possible.
 * 
 * This doesn't use the iterator in the intended way because that wouldn't allow
 * printing of the path to each node.
 * 
 * This initializes an iterator and goes through the whole trie, printing nodes
 * as they are found. If there is a common prefix, that is separated from the
 * rest of the key by a tab character.
 * @param i The current location of the iterator.
 */
void hattrie_print(hattrie_t *T)
{
    hattrie_iter_t* i = malloc_or_die(sizeof(hattrie_iter_t));
    i->T = T;
    i->sorted = true;
